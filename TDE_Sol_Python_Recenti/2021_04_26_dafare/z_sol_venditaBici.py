...
def caricaDatiBici(fn):
    #  return datiBici # se non riuscite ad implementare la funzione, potete usare temporaneamente questa struttura dati
    ...
    for line in fd:
        ...
        while i < len(fields):
            pezzo = fields[i]
            prezzo = int(fields[i + 1])
            li.append((biciID, pezzo, prezzo))
            i += 2
    return li
...
def calcolaCostoMedio(ds):
    ...
    diDen = {}
    for tu in ds:
        biciID, pezzo, prezzo = tu
        if biciID not in diNum:
            diNum[biciID] = 0
            diDen[biciID] = 0
        if pezzo != 'bicicletta':
            diNum[biciID] += prezzo
    ...
    di = {}
    for biciID in diNum:
        if diDen[biciID] != 0:  # controllo non richiesto, lo ho aggiunto per una maggior sicurezza
            di[biciID] = float(diNum[biciID]) / diDen[biciID]  # il float() non e' necessario, e' stato aggiunto per compatibilita' con python2
    return di
...
def numeroCombinazioni(dc):
    ...
    for tu in dc:
        biciID, pezzo, prezzo = tu
        if biciID not in id2ncomp:
            id2ncomp[biciID] = {}
        if pezzo == 'sella':
            #  print( biciID,'\nsella')
            id2ncomp[biciID]['sella'] = 1
        elif pezzo == 'cambio':
            #  print('cambio')
            id2ncomp[biciID]['cambio'] = 1
        elif pezzo == 'manubrio':
            id2ncomp[biciID]['manubrio'] = 1
            #  print('manubrio')
    #  print(id2ncomp)

    nBici = 0
    for biciID in id2ncomp:
        if len(id2ncomp[biciID].keys()) == 3:
            nBici += 1
...
def maggiorIncasso(dc):
    ...
    for tu in dc:
        biciID, pezzo, prezzo = tu
        if pezzo not in pezzo2inc:
            pezzo2inc[pezzo] = 0
        pezzo2inc[pezzo] += prezzo

    # individuo il massimo
    pezzoMax = ''  # per iniziare parto con una stringa vuota
    for pezzo in pezzo2inc:
        inc = pezzo2inc[pezzo]
        if pezzoMax == '' or inc > pezzo2inc[pezzoMax]:
            if pezzo != 'bicicletta':
                pezzoMax = pezzo
    return (pezzoMax, pezzo2inc[pezzoMax])
